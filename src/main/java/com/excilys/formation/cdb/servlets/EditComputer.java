...
public class EditComputer extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			erreurs.put(COMPUTER_ID, e.getLocalizedMessage());
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		ConnexionSQL.getConnexion();
		Computer computer = null;
		System.out.println(req.getAttribute(ATT_COMPUTER));
		Computer computerTest = (Computer) req.getAttribute(ATT_COMPUTER);
		System.out.println(computerTest);
		if(computerTest != null) {
			computerId = "" + computerTest.getId();
			name = computerTest.getName();
			introduced = computerTest.getIntroduced().toString();
			discontinued = computerTest.getDiscontinued().toString();
			companyId = "" + computerTest.getCompany().getId();
		}
		try {
			computer = DAOComputer.getDAOComputer().computerDetails(Integer.parseInt(computerId));
		} catch (NumberFormatException e) {
		...
		} else if(discontinued != null && computer.getIntroduced()!= null) {
			validationChronologieDates(computer.getIntroduced().toString(), discontinued);
		} else if(introduced != null && computer.getDiscontinued()!= null) {
			validationChronologieDates(introduced, computer.getDiscontinued().toString());
		}
		} catch (ValidationException e) {
			erreurs.put(ATT_ERREUR_CHRONOLOGIE, "Le résultat de cette édition donnerait une date introduced plus récente que la date discontinued.");
		}
		try {
			validationCompanyId(companyId);
		} catch(ValidationException e) {}
		
		System.out.println("regilis : " + computerId);
		req.setAttribute(ATT_COMPUTER, computerTest);
	...
	private Computer validationComputerId(String id) throws ValidationException {
		if(id != null && !(id.equals("0")) && !(id.equals("")) && !(id.equals("null")) && com.excilys.formation.cdb.service.Util.stringIsInt(id)) {
			Computer computer;
			try {
