...
//	private String orderByGeneral = "id";
	...
	protected ModelAndView doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		for (int i = 0; i < page.getNombreParPage(); i++) {
			...
			if (valeurBox != null) {
				try {
					System.out.println("BOUAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH");
					logger.debug("La on a ça  :  " + valeurBox);
					logger.debug(computerService.delete(Integer.parseInt(valeurBox)).toString());
